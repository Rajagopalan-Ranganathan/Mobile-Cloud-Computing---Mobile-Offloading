...
const lengthValidatorOptions = { min: 5, max: 1500 }
...
function facebookUser(req, res, next) {
    const email = req.body.email
    const name = req.body.name
    const fbtoken = req.body.fbtoken
    if (validator.isEmail(email) &&
        validator.isLength(fbtoken, lengthValidatorOptions) &&
        validator.isLength(name, lengthValidatorOptions)) {
        userModel.findOne({ email }, (err, user) => {
            if (err) {
                return next(err)
            } else if (!user) {
                const password = fbtoken
                ...
                user.comparePassword(fbtoken).then((isValid) => {
                    if (isValid === true) {
                        const token = jwt.sign(user.toJSON(), process.env.SECRET)
                        res.send({ token })
                    } else {
...
const routes = {
    ...
    POST: { '/auth': authenticateUser, '/facebook': facebookUser },
